...
class ViMode(basemode.BaseMode):
    ...
    def vi_save_line(self):
        ...
        tpl_undo = (self.l_buffer.point, self.l_buffer.line_buffer[:],)
        if not self._vi_undo_stack or self._vi_undo_stack[self._vi_undo_cursor][
            1] != tpl_undo[1]:
    ...
    def vi_search_again_forward(self):
        self.vi_search(list(
            range(self._vi_search_position + 1, len(self._history.history))))
...
class ViCommand:
    ...
    def replace_one(self, char):
        ...
        self.readline.l_buffer.line_buffer[
        cursor: cursor + times] = char * times
        if times > 1:
    ...
    def key_star(self, char):
        ...
        if completions:
            ...
            self.readline.l_buffer.line_buffer[
            self.readline.begidx: self.readline.endidx + 1] = list(text)
    ...
    def key_tilde(self, char):
        ...
        for i in range(self.get_multiplier()):
            try:
                c = self.readline.l_buffer.line_buffer[
                    self.readline.l_buffer.point]
                if c.isupper():
                    self.readline.l_buffer.line_buffer[
                        self.readline.l_buffer.point] = c.lower()
                elif c.islower():
                    self.readline.l_buffer.line_buffer[
                        self.readline.l_buffer.point] = c.upper()
                self.readline.l_buffer.point += 1
            except IndexError:
                break
        self.end()
    ...
    def apply(self):
        if self.motion:
            self.pos_motion = self.motion(self.readline.l_buffer.line_buffer,
                self.readline.l_buffer.point,
    ...
    def end(self):
        ...
        if self.readline.l_buffer.point >= len(
                self.readline.l_buffer.line_buffer):
            self.readline.l_buffer.point = max(0,
                len(self.readline.l_buffer.line_buffer) - 1)
