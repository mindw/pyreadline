...
class BaseMode(object):
    ...
    def rl_settings_to_string(self):
        ...
        out.append(
            tablepat % ("Control", "Meta", "Shift", "Keycode/char", "Function"))
        bindings = [(k[0], k[1], k[2], k[3], v.__name__) for k, v in
            self.key_dispatch.items()]
        bindings.sort()
        for key in bindings:
    ...
    def _get_completions(self):
        ...
        if self.completer:
            ...
            while 1:
                ...
                if r is None:
                    ...
                    pass
            log('text completions=<%s>' % list(
                map(ensure_unicode, completions)))
        if (self.complete_filesystem == "on") and not completions:
            ...
            completions = list(map(ensure_unicode,
    ...
    def complete(self, e):  # (TAB)
        ...
        if completions:
            ...
            if len(cprefix) > 0:
                ...
                self.l_buffer.point = point + len(rep) - (
