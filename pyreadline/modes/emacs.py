...
class EmacsMode(
    ...
    def history_search_forward(self, e):  # ()
        ...
        if self.previous_func and hasattr(
                self._history, self.previous_func.__name__):
            self._history.lastcommand = \
                getattr(self._history, self.previous_func.__name__)
        else:
