...
def next_start_segment(str, is_segment):
    ...
    result[len(result):len(str)] = [len(str) for x in
        range(len(str) - len(result) + 1)]
    return result
...
def prev_start_segment(str, is_segment):
    ...
    prev = 0
    for start in mark_start_segment_index(str, is_segment):
        result[len(result):start + 1] = [prev for x in
            range(start - len(result) + 1)]
        prev = start
    result[len(result):len(str)] = [prev for x in
        range(len(str) - len(result) + 1)]
    return result
